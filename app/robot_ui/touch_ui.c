...
/* 老人友好样式 - 大字体、高对比度 */
static lv_style_t style老人友好;
static lv_style_t style大按钮;
static lv_style_t style菜单项;
static lv_style_t style返回按钮;
static lv_style_t style设置滑块;
static lv_style_t style开关;

/* ==================== 内部函数声明 ==================== */
static void init老人友好样式(void);
...
{
    /* 老人友好基础样式 */
    lv_style_init(&style老人友好);
    lv_style_set_bg_color(&style老人友好, lv_color_hex(0x1A1A2E));
    lv_style_set_bg_opa(&style老人友好, LV_OPA_COVER);
    lv_style_set_text_color(&style老人友好, lv_color_hex(0xFFFFFF));
    lv_style_set_text_font(&style老人友好, &lv_font_montserrat_24); // 大字体
    lv_style_set_border_width(&style老人友好, 0);
    lv_style_set_radius(&style老人友好, 0);

    /* 大按钮样式 - 方便点击 */
    lv_style_init(&style大按钮);
    lv_style_set_bg_color(&style大按钮, lv_color_hex(0x4CAF50));
    lv_style_set_bg_opa(&style大按钮, LV_OPA_COVER);
    lv_style_set_radius(&style大按钮, 20);
    lv_style_set_shadow_width(&style大按钮, 15);
    lv_style_set_shadow_color(&style大按钮, lv_color_hex(0x388E3C));
    lv_style_set_text_color(&style大按钮, lv_color_hex(0xFFFFFF));
    lv_style_set_text_font(&style大按钮, &lv_font_montserrat_20);
    lv_style_set_pad_all(&style大按钮, 20);

    /* 菜单项样式 */
    lv_style_init(&style菜单项);
    lv_style_set_bg_color(&style菜单项, lv_color_hex(0x2D2D44));
    lv_style_set_bg_opa(&style菜单项, LV_OPA_COVER);
    lv_style_set_radius(&style菜单项, 15);
    lv_style_set_border_width(&style菜单项, 2);
    lv_style_set_border_color(&style菜单项, lv_color_hex(0x4CAF50));
    lv_style_set_text_color(&style菜单项, lv_color_hex(0xFFFFFF));
    lv_style_set_text_font(&style菜单项, &lv_font_montserrat_22);
    lv_style_set_pad_all(&style菜单项, 25);

    /* 返回按钮样式 */
    lv_style_init(&style返回按钮);
    lv_style_set_bg_color(&style返回按钮, lv_color_hex(0x607D8B));
    lv_style_set_bg_opa(&style返回按钮, LV_OPA_COVER);
    lv_style_set_radius(&style返回按钮, 25);
    lv_style_set_text_color(&style返回按钮, lv_color_hex(0xFFFFFF));
    lv_style_set_text_font(&style返回按钮, &lv_font_montserrat_18);

    /* 设置滑块样式 */
    lv_style_init(&style设置滑块);
    lv_style_set_bg_color(&style设置滑块, lv_color_hex(0x37474F));
    lv_style_set_radius(&style设置滑块, 10);

    /* 开关样式 */
    lv_style_init(&style开关);
    lv_style_set_bg_color(&style开关, lv_color_hex(0x4CAF50));
}
...
void touch_ui_init(void)
{
    ...
    lv_obj_add_style(current_screen, &style老人友好, 0);

    printf("触摸交互 UI 初始化完成\n");
}
...
static void create_menu_panel(menu_type_t type)
{
    ...
    lv_obj_add_style(menu_panel, &style老人友好, 0);
    ...
    switch (type) {
        ...
        case MENU_TYPE_REMIND:
            lv_label_set_text(title, "⏰ 提醒列表");
            break;
        case MENU_TYPE_SETTING:
            lv_label_set_text(title, "⚙️ 系统设置");
            break;
        case MENU_TYPE_ABOUT:
            lv_label_set_text(title, "ℹ️ 关于");
            break;
        default:
    ...
    switch (type) {
        case MENU_TYPE_MAIN:
            /* 主菜单项 */
            create_menu_item(menu_panel, "🗣️ 语音聊天", "开始与机器人对话", 0);
            create_menu_item(menu_panel, "⏰ 查看提醒", "查看今日提醒", 1);
            create_menu_item(menu_panel, "⚙️ 系统设置", "调整音量、亮度等", 2);
            create_menu_item(menu_panel, "📞 紧急联系", "联系家人", 3);
            create_menu_item(menu_panel, "ℹ️ 关于", "查看版本信息", 4);
            break;
                            ...
                            const char *subtitle, int index)
{
    ...
    lv_obj_add_style(item, &style菜单项, 0);
    lv_obj_set_flex_flow(item, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(item, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_add_event_cb(item, menu_item_event_handler, LV_EVENT_CLICKED, (void *)index);
    ...
    lv_obj_set_style_pad_left(arrow, LV_PCT(100), 0); // 推到右边
}
                                ...
                                const char *time, int index)
{
    ...
    lv_obj_set_flex_flow(item, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(item, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_add_event_cb(item, reminder_item_event_handler, LV_EVENT_CLICKED, (void *)index);
...
static void create_setting_panel(void)
{
    ...
    lv_obj_add_style(setting_panel, &style老人友好, 0);
    ...
    /* 音量设置 */
    create_slider_setting(setting_panel, "🔊 音量", user_settings.volume, 0);

    /* 亮度设置 */
    create_slider_setting(setting_panel, "💡 亮度", user_settings.brightness, 1);

    /* 自动提醒开关 */
    create_switch_setting(setting_panel, "⏰ 自动提醒", user_settings.auto_remind, 2);

    /* 提醒间隔 */
    create_interval_setting(setting_panel, "📅 提醒间隔", user_settings.remind_interval, 3);

    /* 恢复默认设置按钮 */
    lv_obj_t *btn_reset = lv_btn_create(setting_panel);
    lv_obj_set_size(btn_reset, LV_PCT(80), 60);
    lv_obj_add_style(btn_reset, &style返回按钮, 0);
    lv_obj_add_event_cb(btn_reset, setting_reset_event_handler, LV_EVENT_CLICKED, NULL);
    lv_obj_t *lbl_reset = lv_label_create(btn_reset);
    lv_label_set_text(lbl_reset, "🔄 恢复默认设置");
                                 ...
                                 int value, int index)
{
    ...
    lv_obj_set_style_pad_left(value_label, LV_PCT(100), 0);
    ...
    lv_obj_add_style(slider, &style设置滑块, 0);
                                 ...
                                 bool value, int index)
{
    ...
    lv_obj_add_event_cb(sw, setting_switch_event_handler, 
                       LV_EVENT_VALUE_CHANGED, (void *)index);
    lv_obj_set_style_pad_left(sw, LV_PCT(100), 0);
}
...
static void create_back_button(lv_obj_t *parent)
{
    ...
    lv_obj_add_style(btn, &style返回按钮, 0);
...
static void menu_item_event_handler(lv_event_t *e)
{
    ...
    switch (index) {
        ...
        case 2: // 系统设置
            ...
            break;
        case 3: // 紧急联系
            touch_ui_show_setting_detail("紧急联系", "正在呼叫家人...\n请稍候");
            break;
        case 4: // 关于
...
{
    show_confirm_dialog("恢复默认", "确定要恢复默认设置吗？", 
...
{
    int interval = (int)lv_event_get_user_data(e);
                               ...
                               lv_event_cb_t callback)
{
    ...
    lv_obj_t *mbox = lv_msgbox_create(NULL, title, content, btns, true);
    lv_obj_center(mbox);
    ...
    /* 添加按钮事件 */
    lv_obj_t *btn_cancel = lv_msgbox_get_footer_btn(mbox, 0);
    lv_obj_t *btn_confirm = lv_msgbox_get_footer_btn(mbox, 1);
    lv_obj_add_event_cb(btn_cancel, callback, LV_EVENT_CLICKED, (void *)0);
    lv_obj_add_event_cb(btn_confirm, callback, LV_EVENT_CLICKED, (void *)1);
}
...
void touch_ui_set_mode(robot_mode_t mode)
{
    ...
    switch (mode) {
        case MODE_LISTENING:
            /* 显示监听状态 */
            touch_ui_show_setting_detail("语音聊天", "正在聆听...\n请说话");
            break;
        case MODE_SLEEP:
            /* 降低亮度，显示休眠界面 */
            break;
